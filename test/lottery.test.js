...
describe('Lottery',() => { 
    ...
    it('test one account to enter ',async () => {
        ...
        const players =await lottery.methods.getPlayers().call({
            from: accounts[0]
            //Get player มาเช็ว่าแอคเคาท์นั้นเท่ากัน ที่ตำแหน่ง 0 และต้องมีผู้เล่น 1 คน
        });
        assert.strictEqual(accounts[0], players[0]);
        assert.strictEqual(1, players.length);
        //ตรวจสอบ Enter ที่เป็น Payable เวลาเรียกใช้ต้องส่งเงินไป
        //เข้า smart contract เทสเคลลองส่งไป 0.01 ether
    });
    ...
    it('requires minimum amount of ether to enter', async() => {
        ...
        //จงใจให้เกิด error ด้วยการส่ง 0 เข้าไป
        });
        ...
        //ถ้าจะผ่านต้อง not ok
        });   
        //เช็ตว่าเงินเข้ามาถูกต้องไหม
